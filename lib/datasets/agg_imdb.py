...
class AggImdb():
    """
    Aggregation of RepoImdbs by Class 
    """

    def __init__(self, *args, **kwargs):
        imdbList = args
        name,imageset = combineImdbNamesAndImagesets(imdbList)
        cfg.DATASETS.CALLING_DATASET_NAME = name
        cfg.DATASETS.CALLING_IMAGESET_NAME = imageset
        self._cacheStrModifier = kwargs['cacheStrModifier']
        self._path_to_imageSets = kwargs['path_to_imageSets']
    ...
    def combineImdbNamesAndImagesets(imdbList):
        names = ''
        imagesets = ''
        imdbNames = [imdb.name for imdb in imdbList]
        imdbImagesets = [imdb.imagesets for imdb in imdbList]
        indices = sorted(range(len(imdbList)),key=imdbNames.__getitem__)
        for index in indices:
            names = '{}+'.format(imdbNames[index])
            imagesets = '{}+'.format(imdbImageSets[index])
        return names,imagesets
