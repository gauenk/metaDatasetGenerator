...
def parse_args():
    ...
    parser.add_argument('--al_imdb', dest='al_imdb_name',
                        help='imdb dataset to train on',
                        default='pascal_voc-trainval-default', type=str)
...
def get_roidb(imdb_name,args,al=False):
    if al:
        imdb = get_repo_imdb(imdb_name,args.al_new_path_to_imageset)
    else:
...
def get_al_imdb_dataset(args):
    imdb_name = args.al_imdb_name
    imdb,roidb = get_roidb(al_imdb_name,args,al=True)
    output_dir = get_output_dir(imdb)
    print("final roidb_size: {}".format(len(roidb)))
    return imdb,roidb,output_dir
...
if __name__ == '__main__':
    ...
    if args.dataset_type == "imdb":
        ...
        al_imdb,roidb,output_dir = get_al_imdb_dataset
